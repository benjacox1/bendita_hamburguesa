...
function initEventos(){
  ...
  if (btnWA) {
    btnWA.addEventListener('click', async (e) => {
      ...
      if (items.length === 0) {
        ...
        // Aun así abrimos el chat por si quiere consultas
        window.open(btnWA.href, '_blank');
        return;
      }
      // Tomar nombre del input
      ...
      try {
        // Validar carrito y crear pedido PENDIENTE para verificar contra comprobante
        ...
        // Crear pedido con el nombre del cliente
        ...
        // Construir texto dinámico para WhatsApp: conservar texto existente y agregar referencia
        let url;
        try { url = new URL(btnWA.href); } catch { url = null; }
        ...
        // Construir URL del Panel Admin filtrado por ID de pedido
        ...
        // Mostrar toast con resumen rápido
        try {
  ...
  // Limpiar carrito al registrar el pedido (la verificación del pago la hace el Admin con el comprobante)
  ...
  window.open(finalHref, '_blank');
        ...
        const reason = (err && err.message) ? ` Motivo: ${err.message}` : '';
        if (msg) { msg.textContent = 'No se pudo registrar el pedido automáticamente.' + reason + ' Puedes enviar el comprobante por WhatsApp.'; msg.className = 'mini error'; }
        alert('No se pudo registrar el pedido automáticamente.' + reason + '\n\nSe abrirá WhatsApp para que envíes tu comprobante.');
        window.open(btnWA.href, '_blank');
      }
    });
  }
  ...
  if (payLink) {
    payLink.addEventListener('click', (e) => {
      ...
      if (url) {
        // Intento de prellenado (puede ser ignorado por MP si no lo soporta en este tipo de link)
